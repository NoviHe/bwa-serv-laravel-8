...
class AddForeignKeysOrder extends Migration
{
    ...
    public function up()
    {
        Schema::table('order', function (Blueprint $table) {
            ...
            $table->foreign('freelance_id', 'fk_order_freelance_to_users')->references('id')
                ->on('users')->onUpdate('cascade')->onDelete('cascade');

            $table->foreign('service_id', 'fk_order_to_service')->references('id')
                ->on('service')->onUpdate('cascade')->onDelete('cascade');

            $table->foreign('order_status_id', 'fk_order_buyer_to_order_status')->references('id')
    ...
    public function down()
    {
        Schema::table('order', function (Blueprint $table) {
            ...
            $table->dropForeign('fk_order_buyer_to_order_status');
        });
    }
}
